...
import 'html-escaper';
import { $ as $$Lupa, p as products, a as $$Card, b as $$Layout } from './_product_.astro.e0af6fef.mjs';
import 'cookie';
import 'kleur/colors';
...
const $$AllProductsAdmin = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$AllProductsAdmin;
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Nuevo producto" }, { "default": ($$result2) => renderTemplate`
  ...
  <main class="px-4 pb-4 p-4 xl:p-0 lg:max-w-5xl mx-auto">
    <div class="flex flex-col md:flex-row justify-between items-start my-6 gap-4">
      <h1 class="text-2xl font-semibold">Todos los productos</h1>
      <form class="flex py-2 px-4 md:w-36 lg:w-96 items-center bg-zinc-200 rounded-xl">
        <span class="opacity-40 inline-block">
          ${renderComponent($$result2, "Lupa", $$Lupa, {})}
        </span>
        <input placeholder="Buscar item" class="ml-2 bg-transparent focus:border-white focus-visible:outline-none focus:ring-white rounded w-full" type="text" name="name-product" id="product-filter">
      </form>
      <a href="/new-product" class="bg-blue-600 border-blue-600 border-[1px] text-gray-100 font-medium py-2 px-6 rounded block hover:bg-blue-700 hover:text-gray-100">
        Agregar Producto</a>
    </div>
    <ul class="grid grid-cols-box-s gap-x-4 gap-y-5 mb-10 justify-center lg:max-w-6xl">
      ${products.map((item) => renderTemplate`${renderComponent($$result2, "Card", $$Card, { "id": item.id, "name": item.name, "price": item.price, "description": item.description, "image": item.image, "category": item.category, "admin": true })}`)}
    </ul>
  </main>
` })}`;
}, "D:/Dev/alura-geek/src/pages/all-products-admin.astro", void 0);

const $$file = "D:/Dev/alura-geek/src/pages/all-products-admin.astro";
const $$url = "/alura-geek/all-products-admin";

export { $$AllProductsAdmin as default, $$file as file, $$url as url };
